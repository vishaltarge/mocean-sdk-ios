...
- (void)loadAd
{
    NSLog(@"Facebook ad loaded");
    NSString *fb_placementId = [self.adapterDictionary objectForKey:@"adid"];
    [FBAdSettings addTestDevice:@"6b8e93a0479b682b0c876bc576d46880390229ab"];
...
-(void) sendTracker
{
    [self.fb_nativeAd  registerViewForInteraction:self.clickableView withViewController:self.parentViewController ];
}
...
{
    NSLog(@"Facebook Ad was Received sucessfully");
    
    NSLog(@"icon URL : %@ coverImage : %@",[self.fb_nativeAd.icon.url description],[self.fb_nativeAd.coverImage.url description]);
