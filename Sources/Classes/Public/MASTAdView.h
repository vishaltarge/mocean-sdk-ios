...
/** You use the MASTAdView class to embed advertisement content in your application. To do so, you simply create the MASTAdView object and add it to a UIView. An instance of MASTAdView(or simply, an ad) is a mean for displaying advertisement information from ad publisher site. To choose ad publisher site, set parameters in the ad initialization methodinitWithFrame:site:zone: or use the properties site and zone. Use the adServerUrl property to choose ad publisher server.
 
 The Ad handles the rendering of any content in its area: HTML, Video, Gif animation or JavaScript. Ad also handles any interactions with that content. So you can link from the banner to ad publisher site. Use the internalOpenMode property to set open mode for ad publisher site. To control all interactions, implement MASTAdViewDelegate protocol.
 
 Ad always tries to load the content after creation. The time interval between load requests is managed using the property updateTimeInterval. Also you can update ad content immediately using the update method. Use the isLoading property to find out if the ad is in the process of loading or not.
