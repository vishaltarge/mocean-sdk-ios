...
- (void)viewDidLoad
{
    ...
    self.adView = [[AdView alloc] initWithFrame:CGRectMake(0.0, 0.0, self.view.bounds.size.width, 240.0) site:8061 zone:16109];
    self.adView.backgroundColor = [UIColor whiteColor];
    self.adView.updateTimeInterval = 60;
    self.adView.type = AdTypeRichmedia;
    self.adView.logMode = AdLogModeAll;
    [self.view addSubview:self.adView];
    
    self.btnStartTimer = [[UIButton alloc] initWithFrame:CGRectMake(20.0, 280.0, 100.0, 45.0)];
    ...
    self.btnTheWorldMethod = [[UIButton alloc] initWithFrame:CGRectMake(160.0, 280.0, 200.0, 45.0)];
...
- (void)buttonClick:(id)sender {
    if (sender == self.btnStartTimer) {
        //start timer
        [NSTimer scheduledTimerWithTimeInterval:4.0 target:self.adView selector:@selector(stopEverythingAndNotfiyDelegateOnCleanup) userInfo:nil repeats:NO];
